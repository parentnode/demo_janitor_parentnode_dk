...
<div class="scene defaultList <?= $itemtype ?>List">
	...
	<div class="all_items i:defaultList taggable filters sortable"<?= $HTML->jsData() ?>>
<?		if($items): ?>
		<ul class="items">
<?			foreach($items as $item): ?>
			<li class="item image draggable item_id:<?= $item["id"] ?> width:160<?= $HTML->jsMedia($item, "single_media") ?>">
				<div class="drag"></div>
				<h3><?= $item["name"] ?></h3>
