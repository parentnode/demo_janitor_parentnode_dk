...
<? if($categories): ?>
	<div class="categories">
		...
		<? foreach($categories as $tag): ?>
			<li<?= ($selected_tag == $tag["value"] ? ' class="selected"' : '') ?>><a href="/index/tag/<?= urlencode($tag["value"]) ?>"><?= $tag["value"] ?></a></li>
			<? endforeach; ?>
		</ul>
	</div>
<? endif; ?>
...
<?	if($items): ?>
	<ul class="items articles i:articleMiniList">
		...
		<li class="item article id:<?= $item["item_id"] ?>" itemscope itemtype="http://schema.org/NewsArticle"
			...
			<h3 itemprop="headline"><a href="/index/<?= $item["sindex"] ?>"><?= $item["name"] ?></a></h3>


			<?= $HTML->articleInfo($item, "/front/".$item["sindex"], [
				"media" => $media,
				"sharing" => true
			]) ?>
